...
public class AnonRegisterP extends Composite implements MasteryEvents.Listener {
    ...
    void registerClicked(ClickEvent event) {
        ...
        String challenge = getRecaptchaChallenge();
        String response = getRecaptchaResponse();

        String passwordHash = CredentialHelper.hashAnonPassword(emailVal, passwordVal1);

        IMasteryS.Instance.get().authWithNewAnon(challenge, response, emailVal, passwordHash, nameVal, pictureResource.token, new SimpleAsyncCallback<AnonRegisterResultD>() {
            @Override
