...
import _root_.java.io.{File, FileInputStream, InputStream, OutputStream}
import _root_.java.util.regex.Pattern
import _root_.java.util.UUID
import _root_.javax.servlet.http.{HttpServlet, HttpServletRequest, HttpServletResponse}
...
class ResourceS extends HttpServlet {
    ...
    def sendFsFile(resourceId: UUID, part: String, request: HttpServletRequest, response: HttpServletResponse) {
        ...
        try {
            copyStream(fileStream, response.getOutputStream)
            sendPngMeta(fileSize, response)
        }
        finally {
            fileStream.close()
        }
    ...
    def sendDbFile(resourceId: UUID, part: String, request: HttpServletRequest, response: HttpServletResponse) {
        ...
        try {
            val file = Database.loadFile(resourceId, part, os)
            if (file == null) {
                logger.warn("File is empty " + resourceId + " " + part)
                send404(request, response)
                return
            }
            sendFileMeta(file.size, file.contentType, response)
        }
        finally {
            os.close()
        }
    ...
    def sendPngMeta(size: Int, response: HttpServletResponse) {
        sendFileMeta(size, "image/png", response)
    }

    def sendFileMeta(size: Int, contentType: String, response: HttpServletResponse) {
