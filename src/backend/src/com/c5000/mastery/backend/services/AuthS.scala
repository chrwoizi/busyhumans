...
object AuthS extends HasServiceLogger {
    ...
    def authWithNewAnon[Person <: UniqueIdModelBase](remoteAddr: String, recaptchaChallenge: String, recaptchaResponse: String,
        ...
        if (!Config.ENABLE_LOGIN) {
            result.registrationError = "Login is disabled"
        ...
        try {
            ...
            var picture = tokenizer.detokenize(pictureToken).asInstanceOf[ResourceD]
            if (picture == null) {
                if (Config.ENABLE_PICTURES) {
                    ...
                    picture = new ResourceD();
                    picture.resource = "static/default-skill.png";
                    picture.small = picture.resource;
                    picture.medium = picture.resource;
                    picture.large = picture.resource;
